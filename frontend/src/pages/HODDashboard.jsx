...
const HODDashboard = () => {
  ...
  const exportToExcel = () => {
    const exportData = tasks.map(t => ({
      ...
      'Status': t.status,
      'Completed Date': t.taskDoneDate ? format(new Date(t.taskDoneDate), 'yyyy-MM-dd') : 'N/A',
      'Remarks': t.remarks || ''
    }));
  ...
  const pieData = analytics?.tasksByPriority.map(item => ({
    name: item._id,
    value: item.count
  })) || [];
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <StatCard title="Total Tasks" value={analytics?.totalTasks || 0} icon={<Database className="text-indigo-600" />} color="bg-indigo-50" />
          <StatCard title="Completed" value={analytics?.completedTasks || 0} icon={<CheckCircle2 className="text-green-600" />} color="bg-green-50" />
          <StatCard title="Overdue" value={analytics?.overdueTasks || 0} icon={<AlertTriangle className="text-red-600" />} color="bg-red-50" />
          <StatCard title="Teachers" value={teachers.length} icon={<Users className="text-blue-600" />} color="bg-blue-50" />
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="p-6 border-b border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center space-x-2">
              <h3 className="text-xl font-bold text-slate-800">Department Tasks</h3>
              <span className="bg-slate-100 text-slate-600 text-xs px-2.5 py-1 rounded-full font-bold">Excel View</span>
            </div>
            ...
            <table className="excel-table">
              ...
              <tbody className="bg-white divide-y divide-slate-200">
                {filteredTasks.length > 0 ? filteredTasks.map((task) => {
                  ...
                  const nearDeadline = daysLeft >= 0 && daysLeft <= 2 && task.status !== 'Completed';
                  ...
                  return (
                    <tr key={task._id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="excel-td">
                        <span className={`px-2 py-1 rounded text-[10px] font-extrabold uppercase shadow-sm border`} 
                      ...
                      <td className="excel-td">
                        <span className="text-xs">{format(new Date(task.dueDate), 'MMM dd, yyyy')}</span>
                      </td>
                      <td className="excel-td">
                        <span className={`text-xs font-bold ${isOverdue ? 'text-red-500 animate-pulse' : nearDeadline ? 'text-orange-500' : 'text-slate-500'}`}>
                          {task.status === 'Completed' ? '-' : isOverdue ? `${Math.abs(daysLeft)} days overdue` : `${daysLeft} days left`}
                        </span>
                      </td>
                      <td className="excel-td">
                        <span className={`status-badge border ${
                          task.status === 'Completed' ? 'status-completed border-green-200' : 
                          task.status === 'In Progress' ? 'status-progress border-blue-200' : 
                      ...
                      <td className="excel-td">
                        <div className="w-24 bg-slate-100 h-1.5 rounded-full overflow-hidden">
                          <div 
                            ...
                            style={{ width: task.status === 'Completed' ? '100%' : task.status === 'In Progress' ? '50%' : '10%' }}
                          ></div>
                        </div>
                      </td>
                      <td className="excel-td">
            ...
            <div className="relative z-10 inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
              ...
              <form onSubmit={handleCreateTask} className="p-6 space-y-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-1">Days to Complete</label>
                    <input 
                      type="number" 
                      min="1"
                      required
                      ...
                      onChange={(e) => setFormData({...formData, daysToComplete: parseInt(e.target.value)})}
                    />
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1">Initial Remarks (Optional)</label>
                  <textarea 
      ...
      {showTeacherModal && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <div className="fixed inset-0 transition-opacity bg-slate-900/60 backdrop-blur-sm" onClick={() => setShowTeacherModal(false)}></div>
            ...
            <div className="relative z-10 inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
              <div className="bg-blue-600 px-6 py-4 flex justify-between items-center">
                ...
                <button onClick={() => setShowTeacherModal(false)} className="text-white/80 hover:text-white">
                  <Plus className="rotate-45" size={24} />
                </button>
              </div>
